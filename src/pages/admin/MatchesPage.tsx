...
import TeamStatsEntryModal from '../../components/admin/TeamStatsEntryModal';
...
const MatchesPage: React.FC = () => {
	...
	const [showTeamStatsModal, setShowTeamStatsModal] = useState<boolean>(false);
	const [teamStatsMatchId, setTeamStatsMatchId] = useState<number | null>(null);
	const [existingTeamStats, setExistingTeamStats] = useState<TeamMatchStatsComparison | null>(null);
	...
	useEffect(() => {
		const fetchTeams = async () => {
			...
			try {
				...
				handleApiError(err, t, navigate, t('matches.failedToLoadTeams'));
			} finally {
	...
	const fetchMatches = useCallback(async () => {
		...
		try {
			...
			if (!controller.signal.aborted) {
				handleApiError(err, t, navigate, t('matches.failedToLoadMatches'));
				setMatches([]);
				setCount(0);
			}
		} finally {
	...
	}, [page, teamFilter, statusFilter, dateFrom, dateTo, debouncedCompetition, t, navigate]);
	...
	const handleEdit = async (match: MatchListItem) => {
		...
		try {
			...
			handleApiError(err, t, navigate, t('matches.failedToLoadMatchDetails'));
		} finally {
	...
	const confirmDelete = async () => {
		...
		try {
			...
			handleApiError(err, t, navigate, t('matches.failedToDelete'));
		} finally {
			setDeleting(false);
		}
	};
	...
	const handleVideoClick = async (match: MatchListItem) => {
		...
		try {
			...
			handleApiError(err, t, navigate, t('matches.failedToLoadMatchVideo'));
		} finally {
	...
	const handleTeamStatsClick = async (match: MatchListItem) => {
		console.log('=== OPENING TEAM STATS MODAL ===');
		console.log('1. Match ID:', match.id);
		setTeamStatsMatchId(match.id);
		setLoadingTeamStats(true);

		try {
			// Try to load existing team stats
			console.log('2. Fetching existing team stats from /api/stats/team-stats/match/' + match.id + '/comparison/');
			const teamStats = await getTeamStatsComparison(match.id);
			console.log('3. Existing team stats found:', teamStats);
			console.log('4. Home team stats exists?', !!teamStats.home_team_stats);
			console.log('5. Away team stats exists?', !!teamStats.away_team_stats);
			if (teamStats.home_team_stats) {
				console.log('6. Home team stats ID:', teamStats.home_team_stats.id);
			}
			if (teamStats.away_team_stats) {
				console.log('7. Away team stats ID:', teamStats.away_team_stats.id);
			}
			setExistingTeamStats(teamStats);
		} catch (err: any) {
			// No existing stats - that's okay, we'll create new ones
			console.log('3. Error fetching existing team stats');
			console.log('4. Error status:', err?.response?.status);
			console.log('5. Error data:', err?.response?.data);
			console.log('6. Error message:', err?.message);
			console.log('7. Will create new stats');
			setExistingTeamStats(null);
		} finally {
			setLoadingTeamStats(false);
			// Open modal after state is set (use setTimeout to ensure state update)
			setTimeout(() => {
				setShowTeamStatsModal(true);
				console.log('8. Modal opened, existingTeamStats state:', existingTeamStats);
			}, 0);
		}
	...
	const statusOptions = useMemo(() => {
		return [
			{ value: '', label: t('matches.status') + ' - ' + t('common.filter') },
		...
		<DashboardLayout>
			...
			<div className="mb-6">
				<div className="flex items-center justify-between">
					<div>
						<h1 className="text-[28px] font-bold text-gray-900">{t('matches.title')}</h1>
						<p className="text-sm text-gray-600 mt-1">{t('matches.manageMatches')}</p>
					</div>
					<button
				...
				<div className="flex flex-wrap gap-3">
					...
					<div className="relative min-w-[200px]">
						...
						<input
							...
							placeholder={t('matches.competition') + '...'}
			...
			{loading && matches.length === 0 ? (
				...
				<EmptyState
					...
					message={t('matches.noMatchesDescription')}
					action={{
										...
										<div className="flex flex-col gap-2 mt-auto">
											{match.status === 'Completed' && (
												<>
													...
													<button
														onClick={() => handleTeamStatsClick(match)}
														disabled={loadingTeamStats}
														className="flex items-center justify-center gap-2 w-full px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors text-sm font-medium disabled:opacity-50"
													>
														<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<ConfirmDialog
				...
				message={t('common.confirmDelete', { name: t('matches.title') })}
			...
			{/* Team Stats Entry Modal */}
			{teamStatsMatchId && (
				<TeamStatsEntryModal
					isOpen={showTeamStatsModal}
					onClose={() => setShowTeamStatsModal(false)}
					matchId={teamStatsMatchId}
					existingStats={existingTeamStats || undefined}
					onSuccess={handleTeamStatsSuccess}
				/>
			)}
